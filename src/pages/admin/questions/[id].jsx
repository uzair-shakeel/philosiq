...
export default function EditQuestion() {
    ...
    try {
      ...
      if (response.data && response.data.question) {
        setFormData({
          ...
          weight_agree:
            response.data.question.weight_agree ||
            response.data.question.weight ||
            1,
          weight_disagree:
            response.data.question.weight_disagree ||
            response.data.question.weight ||
            1,
          active:
            response.data.question.active !== undefined
              ? response.data.question.active
              : true,
        });
      } else {
        setNotFound(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`/api/questions/${id}`, formData);
  ...
  return (
    <AdminLayout title="Edit Question">
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
        <h1 className="text-2xl font-bold mb-4 md:mb-0">Edit Question</h1>
      </div>
